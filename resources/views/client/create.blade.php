...
<div class="container-fluid py-2">
    <div class="mb-4 d-flex flex-column flex-md-row justify-content-between gap-3 align-items-start">
        <div>
            <h2 class="fs-4 fw-bold text-dark mb-1">Add Client</h2>
            <p class="text-muted small mb-0">Create a client for <strong>{{ $workspace->name }}</strong>.</p>
        </div>
    ...
    <div class="card border-light shadow-sm rounded-4 overflow-hidden">
        <div class="card-body p-3 p-md-4">
            <form method="POST" action="{{ route('clients.store', $workspace) }}">
                @csrf
                ...
                <div class="d-flex gap-2 mt-4">
                    ...
                    <a href="{{ route('clients.index', $workspace) }}" class="btn btn-outline-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </div>
